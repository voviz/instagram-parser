...
def main():
    ...
    while True:
        logins_for_update = parser.run_async_function(parser.on_start)
...
def process_logins(logins_for_update, parser):
    with concurrent.futures.ProcessPoolExecutor(max_workers=settings.PROCESS_COUNT) as executor:
        ...
        futures_stories = executor.submit(parser.run_async_function, parser.get_stories_data, logins_with_id)
        future_ids = executor.submit(parser.run_async_function, parser.get_login_ids_list, logins_without_id)
        future_posts = executor.submit(parser.run_async_function, parser.get_posts_list_by_id, logins_with_id)
